...
class SearchResultsModel: ObservableObject {
    ...
    private var mediaSearchPublisher: AnyPublisher<SRGDataProvider.MediasMatchingQuery.Output, Error>? {
        ...
        return SRGDataProvider.current!.medias(for: applicationConfiguration.vendor, matchingQuery: query, with: searchSettings, pageSize: applicationConfiguration.pageSize, triggeredBy: trigger.triggerable(with: Self.triggerIndex))
    }
}
