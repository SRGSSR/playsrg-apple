    ...
    private var rows: [Row] {
        ...
        case let .loaded(medias: medias):
            if !medias.isEmpty {
                ...
                let item = Content.message(NSLocalizedString("Type to start searching", comment: "Default text displayed when no search criterium has been entered"), iconName: "media-90")
                return [Row(section: .information, items: [item])]
            }
            else {
