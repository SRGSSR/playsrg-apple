...
if [ $# -eq 0 ]; then
  ...
  git diff --staged --name-only --diff-filter=d | grep ".swift$" | xargs -I FILE swiftlint lint --quiet --strict "FILE"
fi
echo "... checking Swift code with SwiftFormat..."
if [ $# -eq 0 ]; then
  swiftformat --lint --quiet . 
elif [[ "$1" == "only-changes" ]]; then
  git diff --staged --name-only --diff-filter=d | grep ".swift$" | xargs -I FILE swiftformat --lint --quiet "FILE"
fi
