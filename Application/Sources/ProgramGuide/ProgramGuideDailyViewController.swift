...
final class ProgramGuideDailyViewController: UIViewController {
    ...
    private func reloadData(for state: ProgramGuideDailyViewModel.State, channel: PlayChannel? = nil) {
        let currentChannel = channel ?? programGuideModel.selectedChannel
...
extension ProgramGuideDailyViewController {
    ...
    func date(atYOffset yOffset: CGFloat) -> Date? {
        ...
        return program.wrappedValue.startDate
    }
...
extension ProgramGuideDailyViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        guard let channel = programGuideModel.selectedChannel,
              let program = dataSource.snapshot().itemIdentifiers(inSection: channel)[indexPath.row].program
        else {
            deselectItems(in: collectionView, animated: true)
            return
        }

        AnalyticsClickEvent.tvGuideOpenInfoBox(program: program.wrappedValue, programGuideLayout: .list).send()
