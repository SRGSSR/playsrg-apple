...
final class ProgramViewModel: ObservableObject {
    ...
    var playAction: (() -> Void)? {
        if let media = currentMedia, media.blockingReason(at: Date()) == .none {
            return {
                guard let sceneDelegate = UIApplication.shared.activeSceneDelegate else { return }
                sceneDelegate.rootTabBarController.play_presentMediaPlayer(with: media, position: nil, airPlaySuggestions: true, fromPushNotification: false, animated: true, completion: nil)
            }
        }
        else {
            return nil
            ...
            action: {
                ...
                if HistoryCanResumePlaybackForMedia(media) {
                    ...
                    alertController.addAction(UIAlertAction(title: NSLocalizedString("Watch from start", comment: "Alert choice to watch content from start"), style: .default, handler: { _ in
                        sceneDelegate.rootTabBarController.play_presentMediaPlayer(with: media, position: .default, airPlaySuggestions: true, fromPushNotification: false, animated: true, completion: nil)
                    }))
                    ...
                    sceneDelegate.rootTabBarController.play_top.present(alertController, animated: true, completion: nil)
                }
                else {
                    sceneDelegate.rootTabBarController.play_presentMediaPlayer(with: media, position: nil, airPlaySuggestions: true, fromPushNotification: false, animated: true, completion: nil)
                }
    ...
    var episodeButtonProperties: ButtonProperties? {
        ...
        return ButtonProperties(
            ...
            action: {
                ...
                sceneDelegate.rootTabBarController.pushViewController(showViewController, animated: false)
                sceneDelegate.window.play_dismissAllViewControllers(animated: true, completion: nil)
            }
        )
    }
