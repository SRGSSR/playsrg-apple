...
* `subtitlesAvailabilityHidden` (optional, boolean): Set to `true` to hide subtitles availability setting and display the label on cells.
* `audioDescriptionAvailabilityHidden` (optional, boolean): Set to `true` to hide audio description availability setting and display the label on cells.
